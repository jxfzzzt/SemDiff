...
public class FieldDeclRenames {
	...
	protected static void renames(DiffCompilationUnit original,
		...
		List<DiffSimpleName> del_declarations = fdn.getNames();

		fdn = new FieldDeclarationNames();
		for (DiffNode add : added) {
			add.accept(fdn);
		}
		List<DiffSimpleName> add_declarations = fdn.getNames();
		...
		for (int i = 0; i < best_add.size(); i++) {
			best_add.get(i).getDifference()
					.setRelatedReference(best_del.get(i));
		}
